...
describe('Products API - GET Operations', () => {
  ...
  describe('GET /products', () => {
    ...
    it('should support pagination with limit and skip parameters', () => {
      ...
      cy.request('GET', `${urls.getAll}?limit=${limit}&skip=${skip}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
        
        // Validate pagination parameters using custom command
        cy.validatePagination(response, limit, skip)
        
        // Validate that we got different products (not the first ones)
        const firstProductId = response.body.products[0].id
        expect(firstProductId).to.be.greaterThan(skip)
      })
    })

    it('should support field selection with select parameter', () => {
      const selectFields = 'title,price,id'
      
      cy.request('GET', `${urls.getAll}?select=${selectFields}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support combined pagination and field selection', () => {
      ...
      cy.request('GET', `${urls.getAll}?limit=${limit}&skip=${skip}&select=${selectFields}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
        ...
        expect(response.body.products).to.have.length(limit)
        ...
        response.body.products.forEach((product: Product) => {
          ...
          expect(product).to.not.have.property('brand')
          expect(product).to.not.have.property('stock')
          expect(product).to.not.have.property('rating')
        })
    ...
    it('should support limit=0 to get all products with field selection', () => {
      ...
      cy.request('GET', `${urls.getAll}?limit=0&select=${selectFields}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support skip with field selection', () => {
      ...
      cy.request('GET', `${urls.getAll}?skip=${skip}&select=${selectFields}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support multiple field selection combinations', () => {
      ...
      fieldCombinations.forEach((fields) => {
        cy.request('GET', `${urls.getAll}?select=${fields}&limit=5`).then((response) => {
          ...
          const expectedFields = selectedFields.includes('id') ? selectedFields : ['id', ...selectedFields]
          ...
          const allFields = ['title', 'description', 'price', 'category', 'brand', 'stock', 'rating']
          const unselectedFields = allFields.filter(field => !selectedFields.includes(field))
    ...
    it('should support sorting by title in ascending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=title&order=asc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting by price in descending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=price&order=desc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting by title in descending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=title&order=desc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting by price in ascending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=price&order=asc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting by rating in descending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=rating&order=desc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting by stock in ascending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=stock&order=asc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting by brand in ascending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=brand&order=asc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
        ...
        const productsWithBrands = products.filter((p: Product) => p.brand !== null && p.brand !== undefined)
        
        // Verify that products with brands are sorted in ascending order
        for (let i = 1; i < productsWithBrands.length; i++) {
          const currentBrand = productsWithBrands[i].brand?.toLowerCase() || ''
          const previousBrand = productsWithBrands[i - 1].brand?.toLowerCase() || ''
        ...
        if (productsWithBrands.length > 0) {
          ...
          const allBrands = productsWithBrands.map((p: Product) => p.brand?.toLowerCase() || '')
          const minBrand = Math.min(...allBrands.map((b: string) => b.charCodeAt(0)))
    ...
    it('should support sorting by category in descending order', () => {
      cy.request('GET', `${urls.getAll}?sortBy=category&order=desc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
        
        // Validate sorting - check if products are sorted alphabetically by category in descending order
        const products = response.body.products
        const categories = products.map((p: Product) => p.category.toLowerCase())
        const sortedCategories = [...categories].sort().reverse()
        
        // Compare first few categories to verify sorting
        expect(categories.slice(0, 5)).to.deep.equal(sortedCategories.slice(0, 5))
      })
    })

    it('should support sorting with pagination', () => {
      cy.request('GET', `${urls.getAll}?sortBy=price&order=desc&limit=10&skip=10`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should support sorting with field selection', () => {
      cy.request('GET', `${urls.getAll}?sortBy=title&order=asc&select=title,price,id`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with very large limit value', () => {
      ...
      cy.request('GET', `${urls.getAll}?limit=${veryLargeLimit}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with skip value equal to total products', () => {
      ...
      cy.request('GET', urls.getAll).then((totalResponse) => {
        ...
        cy.request('GET', `${urls.getAll}?skip=${totalProducts}`).then((response) => {
          // Validate response structure using custom command
          cy.validateProductResponse(response)
    ...
    it('should handle edge case with skip value greater than total products', () => {
      ...
      cy.request('GET', urls.getAll).then((totalResponse) => {
        ...
        cy.request('GET', `${urls.getAll}?skip=${skipGreaterThanTotal}`).then((response) => {
          // Validate response structure using custom command
          cy.validateProductResponse(response)
    ...
    it('should handle edge case with multiple invalid parameters', () => {
      cy.request({
        method: 'GET',
        url: `${urls.getAll}?limit=abc&skip=xyz&sortBy=invalid`,
        failOnStatusCode: false
      }).then((response) => {
        // API returns 400 for invalid parameters
        expect(response.status).to.eq(400)
    ...
    it('should handle edge case with extremely large values', () => {
      ...
      cy.request('GET', `${urls.getAll}?limit=${extremelyLargeValue}&skip=${extremelyLargeValue}`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with empty query parameters', () => {
      cy.request('GET', `${urls.getAll}?limit=&skip=`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with invalid select fields', () => {
      cy.request('GET', `${urls.getAll}?select=invalid_field&limit=5`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with mixed valid and invalid select fields', () => {
      cy.request('GET', `${urls.getAll}?select=title,invalid_field,price&limit=5`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with all parameters combined', () => {
      cy.request('GET', `${urls.getAll}?limit=5&skip=10&select=title,price&sortBy=price&order=desc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with very large values in combined parameters', () => {
      ...
      cy.request('GET', `${urls.getAll}?limit=${veryLargeLimit}&skip=${veryLargeSkip}&select=title,price`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with negative values in combined parameters', () => {
      cy.request('GET', `${urls.getAll}?limit=-5&skip=-10&select=title,price`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with invalid sortBy field', () => {
      cy.request('GET', `${urls.getAll}?sortBy=invalid_field&order=asc`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
    ...
    it('should handle edge case with sorting by numeric fields with null values', () => {
      cy.request('GET', `${urls.getAll}?sortBy=rating&order=desc`).then((response) => {
        ...
        const products = response.body.products
        const validRatings = products.filter((p: Product) => p.rating !== null && p.rating !== undefined)
    ...
    it('should handle edge case with sorting by string fields with null values', () => {
      cy.request('GET', `${urls.getAll}?sortBy=brand&order=asc`).then((response) => {
        ...
        // Validate sorting for non-null values
        const products = response.body.products
        const validBrands = products.filter((p: Product) => p.brand !== null && p.brand !== undefined)
        const brands = validBrands.map((p: Product) => p.brand?.toLowerCase() || '')
    ...
    it('should handle edge case with all sorting parameters combined', () => {
      cy.request('GET', `${urls.getAll}?sortBy=price&order=desc&limit=5&skip=10&select=title,price,id`).then((response) => {
        // Validate response structure using custom command
        cy.validateProductResponse(response)
