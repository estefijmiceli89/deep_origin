...
describe('Products API - Category Products Operations', () => {
  ...
  describe('GET /products/category/{category}', () => {
    ...
    it('should validate products structure for smartphones category', () => {
      cy.request('GET', urls.category('smartphones')).then((response) => {
        ...
        if (products.length > 0) {
          ...
          expect(firstProduct.rating).to.be.at.least(0)
          expect(firstProduct.rating).to.be.at.most(5)
        }
      })
    })

    it('should return products for beauty category', () => {
      cy.request('GET', urls.category('beauty')).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body.products).to.be.an('array')
        expect(response.body.products.length).to.be.greaterThan(0)
    ...
    it('should return products for furniture category', () => {
      cy.request('GET', urls.category('furniture')).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body.products).to.be.an('array')
        expect(response.body.products.length).to.be.greaterThan(0)
    ...
    it('should return products for groceries category', () => {
      cy.request('GET', urls.category('groceries')).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body.products).to.be.an('array')
        expect(response.body.products.length).to.be.greaterThan(0)
        ...
        response.body.products.forEach((product: Product) => {
          expect(product.category).to.eq('groceries')
        })
      })
    })

    it('should return products for laptops category', () => {
      cy.request('GET', urls.category('laptops')).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body.products).to.be.an('array')
        expect(response.body.products.length).to.be.greaterThan(0)
        ...
        response.body.products.forEach((product: Product) => {
          expect(product.category).to.eq('laptops')
        })
      })
    })
    ...
    it('should validate products against categories endpoint', () => {
      ...
      cy.request('GET', urls.categories).then((categoriesResponse) => {
        ...
        testCategories.forEach((category: any) => {
          cy.request('GET', urls.category(category.slug)).then((productsResponse) => {
    ...
    it('should handle non-existent category gracefully', () => {
      cy.request({
        method: 'GET',
        url: urls.category('non-existent-category'),
    ...
    it('should handle empty category parameter', () => {
      cy.request({
        ...
        expect(response.status).to.be.oneOf([200, 301, 302, 404, 400])
      })
    })
    ...
    it('should handle category with spaces', () => {
      cy.request({
        method: 'GET',
        url: urls.category('smart phones'),
        failOnStatusCode: false
      }).then((response) => {
        expect(response.status).to.be.oneOf([200, 404, 400])
      })
    ...
    it('should handle very long category name', () => {
      ...
      cy.request({
        method: 'GET',
        url: urls.category(longCategory),
        failOnStatusCode: false
      }).then((response) => {
        expect(response.status).to.be.oneOf([200, 404, 400])
      })
    ...
    it('should handle SQL injection attempt in category', () => {
      cy.request({
        method: 'GET',
        url: urls.category("smartphones' OR '1'='1"),
        failOnStatusCode: false
      }).then((response) => {
        expect(response.status).to.be.oneOf([200, 404, 400])
      })
    ...
    it('should handle XSS attempt in category', () => {
      cy.request({
        method: 'GET',
        url: urls.category('<script>alert("xss")</script>'),
        failOnStatusCode: false
      }).then((response) => {
        expect(response.status).to.be.oneOf([200, 404, 400])
      })
    ...
    it('should validate response structure consistency', () => {
      ...
      for (let i = 0; i < 3; i++) {
        requests.push(
          cy.request('GET', urls.category('smartphones'))
        )
      }
    ...
    it('should validate category consistency with search endpoint', () => {
      ...
      cy.request('GET', urls.category(category)).then((categoryResponse) => {
        ...
        cy.request('GET', `${urls.search}?q=${category}`).then((searchResponse) => {
          ...
          const commonProducts = categoryProductIds.filter((id: number) => searchProductIds.includes(id))
