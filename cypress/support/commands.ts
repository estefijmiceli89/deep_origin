...
Cypress.Commands.add('validateProductResponse', (response: Cypress.Response<ProductsResponse>) => {
...
Cypress.Commands.add('checkResponseTime', (response: Cypress.Response<any>, maxTime: number = 3000) => {
...
Cypress.Commands.add('validatePagination', (response: Cypress.Response<ProductsResponse>, expectedLimit: number, expectedSkip: number = 0) => {
...
Cypress.Commands.add('validateFieldSelection', (product: Product, includedFields: string[], excludedFields: string[]) => {
...
Cypress.Commands.add('validateSingleProductResponse', (response: Cypress.Response<Product>) => {
  expect(response.status).to.eq(200)
...
Cypress.Commands.add('validateSearchResponse', (response: Cypress.Response<ProductsResponse>) => {
  expect(response.status).to.eq(200)
  expect(response.body).to.have.property('products')
  expect(response.body).to.have.property('total')
  expect(response.body).to.have.property('skip')
...
Cypress.Commands.add('validateProductMatchesSearch', (products: Product[], searchQuery: string) => {
  products.forEach(product => {
    const brandText = product.brand || ''
    const searchableText = `${product.title} ${product.description} ${product.category} ${brandText}`.toLowerCase()
    expect(searchableText).to.include(searchQuery.toLowerCase())
  })
})

// Custom command to validate product details against fixture
Cypress.Commands.add('validateProductAgainstFixture', (product: Product, fixtureData: Product) => {
...
Cypress.Commands.add('validateGroceryProductAgainstFixture', (product: Product, fixtureData: Product) => {
  expect(product.id).to.eq(fixtureData.id)
  expect(product.title).to.eq(fixtureData.title)
  expect(product.description).to.eq(fixtureData.description)
  expect(product.price).to.eq(fixtureData.price)
...
Cypress.Commands.add('validateBasicProductData', (product: Product, fixtureProduct: Product) => {
...
Cypress.Commands.add('validatePaginationAgainstFixture', (response: Cypress.Response<ProductsResponse>, fixtureData: ProductsResponse) => {
